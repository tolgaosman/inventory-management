...
export function WarehouseStockBars({ data }: { data: WarehouseStockTotal[] }) {
  ...
  return (
    <PanelCard title="Depo Bazında Stok" meta={`${data.length} depo`}>
      {data.length === 0 ? (
              ...
              return (
                <div key={w.warehouseId}>
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                    {/* Fill colour tracks how full the warehouse is — a bar that is
                        always green tells you nothing you couldn't read off the number. */}
                    <div
                      className={cn("h-full rounded-full transition-all duration-300", capacityIndicatorClass(percent))}
